<template>
  <div class="tile">
    ...
    <div
      :class="{'percentage': true, 'positive': isPositive, 'negative': !isPositive}"
      v-if="props.percentage !== 0"
    >
      <div :class="[`dx-icon-${isPositive ? 'spinup' : 'spindown'}`]" />
      <div class="value">
        {{ Math.abs(percentage) }}%
      </div>
    </div>
...
const getIconClass = () => props.tone || (props.isPositive ? 'positive' : 'negative');
</script>
